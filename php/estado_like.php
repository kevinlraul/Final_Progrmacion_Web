...
if (isset($postdata) && !empty($postdata)) {
	...
	$sql = "SELECT DISTINCT me_gusta FROM like_pelicula where pelicula_id= " . intval($pelicula_id) . " and user_id= " . intval($user_id) . "  LIMIT  1; ";
...
if (isset($_GET['pelicula_id']) && isset($_GET['user_id'])) {
    ...
    $sql = "SELECT DISTINCT me_gusta FROM like_pelicula WHERE pelicula_id = $pelicula_id AND user_id = $user_id LIMIT 1";
